...
const AccountPage = () => {
  ...
  const performTransaction = (amount: number | '') => {
    const asyncFun = async () => {
      ...
      window.alert(
        `Transaction Success! ₩${NBalance} -> ₩${data.balance}\nThank you for using SPARCS Bank`
      );
      setNTransaction(0);
      setNBalance(data.balance);
    };
    ...
    <div className={'account'}>
      ...
      <div className={'account-bank'}>
        ...
        <div className={'transaction'}>
          ...
          />
          <button onClick={(e) => performTransaction(NTransaction)}>
            DEPOSIT
          </button>
          <button
            onClick={(e) =>
              performTransaction(NTransaction === '' ? '' : NTransaction * -1)
            }
          >
            WITHDRAW
          </button>
